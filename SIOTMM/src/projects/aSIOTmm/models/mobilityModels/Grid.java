...
public class Grid {
	...
	public Grid(int x_dim, int y_dim, int side_num_tiles) {
		...
		//grid_cells = new Vector<Cell>((int) Math.pow(side_num_tiles, 2));

		//System.out.println("cells_map.size="+cells_map.capacity());
		//setCellCenter();
	}
	...
	/*private void setCellCenter() {
		...
		for(int i = 0; i<Math.pow(side_num_tiles, 2); i++) {
			Cell c_tmp = new Cell();
			
			p_center = getCellCenterPositionByID(i);
			//System.out.println("p_center  ="+p_center);
			
			c_tmp.xCoord = p_center.xCoord;
			c_tmp.yCoord = p_center.yCoord;
			c_tmp.zCoord = p_center.zCoord;
			//System.out.println("cells_map.get(i)  ="+c_tmp);
			
			grid_cells.add(c_tmp);
			
		}
		
		//System.out.println("grid_cells="+grid_cells.get(0));
		//System.out.println("grid_cells="+grid_cells.get(grid_cells.size()-1));
	}*/
	...
	public List<Integer> getNeighbouringCellList(int cell_home,  int radius){
		...
		//System.out.println("cell_home_id="+cell_home+" home_line="+home_line+" home_col="+home_col);
...
//		System.out.println("top_left_col="+top_left_col+"  top_right_col="+top_right_col);
//		
//		System.out.println("top_left_line="+top_left_line+"  bottom_left_line="+bottom_left_line);
		...
		/*List<Integer> neighborhood = new ArrayList<Integer>();
		int i, j, top_left, tile_id;
		
		top_left = cell_home - ((num_tiles+1)*radius);
		
		for(i=0; i <= Math.pow(2, radius); i++) {
			tile_id = top_left + (num_tiles * i);
			
			if(checkBoundary(tile_id, cell_home, radius)) {
				neighborhood.add(tile_id);
			}
			
			
			for(j=1; j<=Math.pow(2, radius); j++) {
				tile_id = top_left + (num_tiles * i) + j;
	...
	/*private boolean checkBoundary(int id_tocheck, int cell_home,  int radius) {
		
		//if(id_tocheck < 0 || id_tocheck > Math.pow(num_tiles, 2)) return false;
